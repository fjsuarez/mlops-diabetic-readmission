...
MLflow-compatible, modular data validation step with Hydra config, W&B logging,
...
from data_validation.data_validator import validate_data  # noqa: E402
...
def _html_from_report(report: Dict[str, Any]) -> str:
    ...
    lines.append(
        f"<p>Errors: {len(report.get('errors', []))} |\
             Warnings: {len(report.get('warnings', []))}</p>"
    )
...
def main(cfg: DictConfig) -> None:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Load config dict first to get CSV reading parameters
            with open(config_path, "r") as f:
                config_dict = yaml.safe_load(f)

            # Get CSV reading parameters from config
            ...
            df = pd.read_csv(
                os.path.join(raw_path, "raw_data.csv"),
                **csv_params
            )
